...
def output_control(form,dirname):
	...
	if plot_opt == "time_opts":
		...
		print '<h2>Plotting error</h2>'
		print '<p> Unknown plotting option </html>'
		exit()
	...
	try:
		outvar = []
	        ...
	        print '<html><h2>Output error</h2>'
                print '<h3> Cannot read output file: '+filename+'</h3>'
                print "<h4>The file may be corrupted, or your session may have expired. </h4>"
                print "<p>If you leave the model idle for longer than 10 minutes its output may be cleaned up.</html>"
                exit()
