...
C
      PARAMETER (NA=50, NAP=NA+1, NTIME=10000, NCL=1)
...
C
	...
	REAL HOUR0, HOURE
	REAL QCONDT(NA)
	CHARACTER*1 RADINT, TSINT, CALB
C -- twc: 2013-03-08 - replaced TDEP/DDEP/ANRAD/DARAD with a single
C variable, 'RADFLAG'
C value / physical meaning
C 0     / planetary-average radiation: SCON'= 3/8*SCON; COSZETA = 2/3
C 1     / annual+diurnal average rad. for given lat.
C 2     / diurnal avg. rad on initial date
C 3     / diurnally-varying rad. on initial date
C 4     / diurnally and annually varying radiation
        INTEGER RADFLAG
        REAL SCON, SCONEFF
...
C
C MS: add to the params.in file the name of the output directory
C Limit to 30 characters because f77 is ANCIENT.
...
C 
C MS: Read the output directory from the command line
C
        NDIR = IARGC()
        IF (NDIR .EQ. 0) THEN
           OUTDIR = 'output'
...
C
      PRINT*, "Running model in directory: ", OUTDIR
      OPEN(UNIT=8,FILE=OUTDIR(1:NDIR)//'/params.in',
     1     STATUS='OLD')
      OPEN(UNIT=11,FILE=OUTDIR(1:NDIR)//'/error.out',
     1   STATUS='UNKNOWN')
c MS: add log file
      OPEN(UNIT=22,FILE=OUTDIR(1:NDIR)//'/log.out',
     1   STATUS='UNKNOWN',POSITION='REWIND')
...
C
	IF(RESTART.EQ.'Y'.OR.RESTART.EQ.'y')THEN
	 OPEN(UNIT=9,FILE=OUTDIR(1:NDIR)//'/sounding.out',
     1        STATUS='UNKNOWN')
	ELSE
       OPEN(UNIT=9,FILE=OUTDIR(1:NDIR)//'/sounding.in',STATUS='OLD')
	END IF
...
C
	IF(RADFLAG.EQ.0.OR.RADFLAG.EQ.1)THEN
	  distm=0.0
	  rmumean=0.0
	  fracmean=0.0
	  albmean=0.0
...
C  MS: this is now a reference profile rather than profile from last run
C
	IF(ICLDS.EQ.'n'.OR.ICLDS.EQ.'N')THEN
C
C MS: change this to read a reference profile
C     Sorry about this terrible hardcode of the path. Should fix this at some point
	 OPEN(UNIT=9,FILE='../model/profile.reference',STATUS='OLD')
	 DO I=1,NP
...
C
        DO I = 1, NP
          CLDFRAD(I) = CLDF(I)
          CLDQRAD(I) = CLDQ(I)
C -- twc: 2013-03-08 -- add option for no-cloud-radiation interactions
C (ICLDS=0) in params.in file
          IF(ICLDS.EQ.'0')THEN
            CLDFRAD(I) = 0.0
            CLDQRAD(I) = 0.0
          END IF 
...
C
C -- twc: 2013/03/08- using 'RADFLAG' for time-dependence of radiation
C -- see notes above where RADFLAG is declared
C
C begin loop over RADFLAG options
        IF(RADFLAG.EQ.0)THEN
C -- twc: 2013/03/08 - in the case of RADFLAG=0, set an effective colar
C constant to 3/8*SCON, and fix the cos(zenith angle) factor to 2/3
C These two definitions, along with dist=1.0 and fract=1.0, give a TOA
C insolation of S0/4, as is appropriate for the planetary-average 
          SCONEFF=SCON*0.375
          dist=1.0
          rmu0=0.66667
          fract=1.0           
...
c
C -- twc: 2013/03/08- only vary julian day if RADFLAG=4 
	    IF(RADFLAG.EQ.4)THEN
...
c
            IF(RADFLAG.EQ.2)THEN ! no diurnal cycle
              CALL angle(zlongi,RLAT,fract,rmu0)
              CALL alboc(FLOAT(julien),RLAT,albsol)
            ELSE
	      IF(RADFLAG.EQ.3.OR.RADFLAG.EQ.4)THEN
...
c
        END IF
C (end loop over RADFLAG options)
...
c
     ...
     e             SCONEFF,paprs, pplay,TS2,albsol,T2,RV2,O3*O3M,
...
C
C MS: output time in days to status file
C
           REWIND(22)
           WRITE(22,'(F12.3)') TIME/ENDTIME
           FLUSH(22)
...
C
      OPEN(UNIT=10,FILE=OUTDIR(1:NDIR)//'/sounding.out',
     1   STATUS='UNKNOWN')
C
      TS2C=TS2(1)-273.15
...
c	   RM=1000.*RHTEMP(I)*RS(I)
  ...
  260	CONTINUE	
      CLOSE(10)
C
      OPEN(UNIT=12,FILE=OUTDIR(1:NDIR)//'/time.out',
     1   STATUS='UNKNOWN')
      OPEN(UNIT=14,FILE=OUTDIR(1:NDIR)//'/profile.out',
     1   STATUS='UNKNOWN')
      OPEN(UNIT=33,FILE=OUTDIR(1:NDIR)//'/table.out',
     1   STATUS='UNKNOWN')
	OPEN(UNIT=17,FILE=OUTDIR(1:NDIR)//'/plabel.out',
     1   STATUS='UNKNOWN')
C
...
C
	...
	3  WATERBAR(I,2),WATERBAR(I,1),TP(I)
        ...
        WRITE(33,316)P(I),TC,QG,RHBAR(I)
...
C
  ...
  322 FORMAT(50(1X,F6.4))
	CLOSE(15)
C
	OPEN(UNIT=15,FILE=OUTDIR(1:NDIR)//'/thov.out',STATUS='UNKNOWN')
	DO I=1,NPR
...
C
	OPEN(UNIT=15,FILE=OUTDIR(1:NDIR)//'/qhov.out',STATUS='UNKNOWN')
	DO I=1,NPR
	 WRITE(15,323)(QHOV(I,J),J=1,NP)
	END DO
	CLOSE(15)
C
	OPEN(UNIT=15,FILE=OUTDIR(1:NDIR)//'/rhhov.out',STATUS='UNKNOWN')
	DO I=1,NPR
	 WRITE(15,323)(RHHOV(I,J),J=1,NP)
	END DO
	CLOSE(15)
C
	OPEN(UNIT=15,FILE=OUTDIR(1:NDIR)//'/mhov.out',STATUS='UNKNOWN')
	DO I=1,NPR
	 WRITE(15,323)(MHOV(I,J),J=1,NP)
	END DO
	CLOSE(15)
C
	OPEN(UNIT=15,FILE=OUTDIR(1:NDIR)//'/mdhov.out',STATUS='UNKNOWN')
	DO I=1,NPR
	 WRITE(15,323)(MDHOV(I,J),J=1,NP)
	END DO
	CLOSE(15)
C
	OPEN(UNIT=15,FILE=OUTDIR(1:NDIR)//'/mphov.out',STATUS='UNKNOWN')
	DO I=1,NPR
	 WRITE(15,323)(MPHOV(I,J),J=1,NP)
	END DO
	CLOSE(15)
C
	OPEN(UNIT=15,FILE=OUTDIR(1:NDIR)//'/omhov.out',STATUS='UNKNOWN')
	DO I=1,NPR
